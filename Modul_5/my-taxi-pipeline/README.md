...
### Windows: "Cannot locate timezone 'UTC': Timezone database not found"

On Windows, PyArrow (used by Bruin/ingestr) requires an IANA timezone database. If you see this error when running `ingestion.trips`:

1. **Install tzdata in the ingestr environment:**
   ```powershell
   uv tool install ingestr --with tzdata --force
   ```

2. **Run the pipeline with TZDIR set** (use the provided script):
   ```powershell
   .\run-pipeline.ps1 --full-refresh --start-date 2022-01-01 --end-date 2022-02-01
   ```

   Or set TZDIR manually before running bruin:
   ```powershell
   $env:TZDIR = "C:\Users\USER\AppData\Roaming\uv\tools\ingestr\Lib\site-packages\tzdata"
   bruin run ./pipeline/pipeline.yml --full-refresh --start-date 2022-01-01 --end-date 2022-02-01
   ```

   (Adjust the path if tzdata is installed elsewhere; find it with `python -c "import tzdata, os; print(os.path.dirname(tzdata.__file__))"`)
